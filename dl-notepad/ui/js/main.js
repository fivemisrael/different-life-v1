    ...
    window.addEventListener('message', function (event) {
            ...
            if (event.data.type === "showpage") {
                ...
                notepad.on("click", "#drop-note", function () {
                    ...
                    if (data !== "") {
                        $.post('http://dl-notepad/getNote', JSON.stringify(data));
                        $('.moveable').css('display', 'none');
                        $.post('http://dl-notepad/NUIFocusOff', JSON.stringify({}));
                        notetxt.val("");
                    } else {
                        notepad.css('display', 'none');
                        $('.moveable').css('display', 'none');
            ...
            if (event.data.type === "pickup") {
                ...
                notepad2.on("click", "#destroy-note", function () {
                    ...
                    $('.moveable').css('display', 'none');
                    $.post('http://dl-notepad/NUIFocusOff', JSON.stringify({}));
                    // $.post('http://dl-notepad/destroynote', JSON.stringify(event.data.noteid));
                });
                ...
                notepad2.on("click", "#drop-note2", function () {
                    ...
                    if (notetxt2.val() !== "") {
                        $.post('http://dl-notepad/getNote', JSON.stringify(event.data.notetxt));
                        notetxt2.val("");
                    }
                });
            }
