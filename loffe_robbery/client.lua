...
RegisterNetEvent('dl-robbery:onPedDeath')
AddEventHandler('dl-robbery:onPedDeath', function(store)
    SetEntityHealth(peds[store], 0)
end)

RegisterNetEvent('dl-robbery:msgPolice')
AddEventHandler('dl-robbery:msgPolice', function(store, robber)
...
RegisterNetEvent('dl-robbery:removePickup')
AddEventHandler('dl-robbery:removePickup', function(bank)
    for i = 1, #objects do 
...
RegisterNetEvent('dl-robbery:robberyOver')
AddEventHandler('dl-robbery:robberyOver', function()
    robbing = false
end)

RegisterNetEvent('dl-robbery:talk')
AddEventHandler('dl-robbery:talk', function(store, text, time)
    robbing = false
...
RegisterNetEvent('dl-robbery:rob')
AddEventHandler('dl-robbery:rob', function(i)
                    ...
                    while true do
                        ...
                        if DoesEntityExist(bag) then
                            if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), GetEntityCoords(bag), true) <= 1.5 then
                                ...
                                TriggerServerEvent('dl-robbery:pickUp', i)
                                break
                            end
                        else
                            break
...
RegisterNetEvent('dl-robbery:resetStore')
AddEventHandler('dl-robbery:resetStore', function(i)
...
Citizen.CreateThread(function()
    ...
    Citizen.CreateThread(function()
        while true do
            for i = 1, #peds do
                if IsPedDeadOrDying(peds[i]) then
                    TriggerServerEvent('dl-robbery:pedDead', i)
                end
            end
            Wait(5000)
        end
            ...
            if IsPlayerFreeAiming(PlayerId()) then
                for i = 1, #peds do
                    if HasEntityClearLosToEntityInFront(me, peds[i], 19) and not IsPedDeadOrDying(peds[i]) and GetDistanceBetweenCoords(GetEntityCoords(me), GetEntityCoords(peds[i]), true) <= 5.0 then
                        if not robbing then
                            local canRob = nil
                            ESX.TriggerServerCallback('dl-robbery:canRob', function(cb)
                                canRob = cb
                            end, i)
                            while canRob == nil do
                                Wait(0)
                            ...
                            if canRob == true then
                                ...
                                if GetDistanceBetweenCoords(GetEntityCoords(me), GetEntityCoords(peds[i]), true) <= 7.5 then
                                    if not IsPedDeadOrDying(peds[i]) then
                                        TriggerServerEvent('dl-robbery:rob', i)
                                        while robbing do Wait(0) if IsPedDeadOrDying(peds[i]) then robbing = false end end
                                    end
                                else
                                    ClearPedTasks(peds[i])
                            ...
                            elseif canRob == 'no_cops' then
                                ...
                                TriggerEvent('dl-robbery:talk', i, '~g~*' .. Translation[Config.Locale]['shopkeeper'] .. '* ~w~' .. Translation[Config.Locale]['robbed'], 5)
                                Wait(2500)
                            end
                        end
                    end
