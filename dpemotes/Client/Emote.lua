...
Citizen.CreateThread(function()
  while true do
    ...
    if IsPedShooting(playerPed) and IsInAnimation then
      EmoteCancel()
    end
	...
	if IsControlJustPressed(0, 207) then -- Page Down
		...
		local ret, ent = FindFirstObject()
		local success = true
		while success do
			if IsEntityAttachedToEntity(ent, playerPed) then
				SetEntityAsMissionEntity(ent, true, true)
				DeleteEntity(ent)
			end
			success, ent = FindNextObject(ret)
		end
		EndFindObject(ret)
	end
    Citizen.Wait(1)
