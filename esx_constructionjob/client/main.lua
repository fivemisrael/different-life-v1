...
local index = GetRandomIntInRange(1, #Config.Construction)

for k, v in pairs(Config.Zones) do
  if v.Pos.x == Config.Construction[index].x and v.Pos.y == Config.Construction[index].y and v.Pos.z == Config.Construction[index].z then
    return k
  end
end
end
...
if cancel then
  ...
  TriggerServerEvent('esx_constructionjob:GiveItem')
  StartNPCJob()
  Done = true
end
end
...
AddEventHandler('esx_constructionjob:hasEnteredMarker', function(zone)
...
AddEventHandler('esx_constructionjob:hasExitedMarker', function(zone)

if zone == 'Vente' then
TriggerServerEvent('esx_constructionjob:stopVente')
end
CurrentAction = nil
ESX.UI.Menu.CloseAll()
end)
...
TriggerEvent('esx_constructionjob:hasEnteredMarker', currentZone)
end
if not isInMarker and HasAlreadyEnteredMarker then
HasAlreadyEnteredMarker = false
TriggerEvent('esx_constructionjob:hasExitedMarker', LastZone)
end
end
end
end)
...
TriggerServerEvent('esx_constructionjob:startVente')
end
...
if IsControlJustReleased(1, Keys["8"]) and PlayerData.job ~= nil and PlayerData.job.name == Config.nameJob then
