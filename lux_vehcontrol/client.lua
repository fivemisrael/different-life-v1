...
Citizen.CreateThread(function()
	local timer = 0
	local vehs = {[tostring(GetHashKey("k9rb"))] = true, [tostring(GetHashKey("k9chargersaleredblue"))] = true, [tostring(GetHashKey("k9exprb"))] = true}
	while true do
			...
			if timer < tick then
				...
				while suc do
					if DoesEntityExist(veh) then
						local model = tostring(GetEntityModel(veh))
						if vehs[model] then
							for i = 1, 12 do
