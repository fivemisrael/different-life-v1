...
function CloseInstance()
	...
	TriggerEvent("tokovoip:setInstance",false)
end

function EnterInstance(instance)
	InsideInstance = true
	TriggerEvent("tokovoip:setInstance",true)
...
function LeaveInstance()
	...
	TriggerServerEvent('instance:leave', Instance.host or "none")
			...
			for _, player in ipairs(GetActivePlayers()) do
			    ...
			    for j=1, #Instance.players, 1 do
					...
					if player == instancePlayer then
						found = true
						TriggerEvent("tokovoip:addInstancePlayer",player)
					end
				end
				if not found then
					local otherPlayerPed = GetPlayerPed(player)
					TriggerEvent("tokovoip:removeInstancePlayer",player)
