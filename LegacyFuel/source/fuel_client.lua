...
function ManageFuelUsage(vehicle)
	...
	if IsVehicleEngineOn(vehicle) then
		--print(tostring(vehicle), tostring(GetVehicleFuelLevel(vehicle)), tostring(Config.FuelUsage[Round(GetVehicleCurrentRpm(vehicle), 1)]), tostring(Config.Classes[GetVehicleClass(vehicle)]))
		if vehicle and GetVehicleFuelLevel(vehicle) and Config.FuelUsage[Round(GetVehicleCurrentRpm(vehicle), 1)] and Config.Classes[GetVehicleClass(vehicle)] then
	...
	while true do
		...
		if IsPedInAnyVehicle(ped) then
			...
			if not inBlacklisted and GetPedInVehicleSeat(vehicle, -1) == ped then
				if vehicle then ManageFuelUsage(vehicle) end
			end
		else
			if fuelSynced then
				fuelSynced = false
