...
AddEventHandler('instance:onEnter', function(instance)
    if instance.type == 'motelroom' then
    	TriggerServerEvent("InteractSound_SV:PlayOnSource", "lockdoor", 1.0)
...
AddEventHandler('lsrp-motels:roomOptions', function(room, motel)
    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'lsrp-motels',
        {
            ...
            align    = 'top-left',
            elements = { 
                { label = 'הכנס לחדר', value = 'enter' },
                { label = 'ביטול שכירות', value = 'cancel' }
            }
        },
    function(data, entry)
...
AddEventHandler('lsrp-motels:roomMenu', function(room, motel)
    ...
    for k,v in pairs(Config.Zones) do
        ...
        table.insert(options, {label = 'שינוי דמות', value = 'changechar'})
        end
        table.insert(options, {label = 'עזוב את החדר', value = 'leaveroom'})
        if roomOwner == playerIdent then
        table.insert(options, {label = 'פתח את אחסון החדר', value = 'inventory'})
       -- table.insert(options, {label = 'הזמן שחקן', value = 'inviteplayer'})
        end
...
function roomMarkers()
    ...
    for k,v in pairs(Config.Zones) do
        ...
        if distance < 1.0 then
            DrawText3D(v.Menu.x, v.Menu.y, v.Menu.z + 0.35, 'Press [~b~E~s~] to access menu.')
    ...
    for k,v in pairs(Config.Zones) do
        ...
        if distance < 1.0 then
            if roomOwner == playerIdent then
            DrawText3D(v.Inventory.x, v.Inventory.y, v.Inventory.z + 0.35, 'Press [~b~E~s~] to change outfit.')
                if IsControlJustReleased(0, Keys['E']) then
                    PlayerDressings()
                end
            end
    ...
    for k,v in pairs(Config.Zones) do
        ...
        if distance < 1.0 then
            if roomOwner == playerIdent then
            DrawText3D(v.BedStash.x, v.BedStash.y, v.BedStash.z + 0.1, 'Press [~b~E~s~] to access stash.')
                if IsControlJustReleased(0, Keys['E']) then
    ...
    for k,v in pairs(Config.Zones) do
        for km,vm in pairs(v.Rooms) do
            if vm.instancename == myMotel then
                ...
                if (distance < 1.0) then
                    DrawText3D(vm.entry.x, vm.entry.y, vm.entry.z + 0.35, 'Press [~b~E~s~] for options.')
                    if IsControlJustReleased(0, Keys['E']) then
        ...
        for k,v in pairs(Config.Zones) do
            ...
            if (distance < v.Boundries) then
                for km,vm in pairs(v.Rooms) do
                    distance = GetDistanceBetweenCoords(coords, vm.entry.x, vm.entry.y, vm.entry.z, true)
                    if (distance < 1.0) then
                        DrawText3D(vm.entry.x, vm.entry.y, vm.entry.z + 0.35, 'Press [~b~E~s~] to rent Room ~b~'..vm.number..' ~w~for $~b~'..Config.PriceRental)
                        if IsControlJustReleased(0, Keys['E']) then
