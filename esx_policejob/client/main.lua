...
function OpenCloakroomMenu()
	...
	local elements = {
		...
		{ label = _U('bullet_wear'), value = 'Pbullet_wear' },
...
if PlayerData.job.grade_name == 'boss' or PlayerData.job.grade_name == 'lieutenant' or PlayerData.job.grade_name == 'officer' then
...
function OpenGetWeaponStorageMenu()
	...
	ESX.TriggerServerCallback('y5kwaa:getStorageWeapons', function(weapons)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'storage_get_weapon',
		{
			...
			ESX.TriggerServerCallback('y5kwaa:removeStorageWeapon', function()
				OpenGetWeaponStorageMenu()
...
function OpenPutWeaponStorageMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'storage_put_weapon',
	{
		...
		ESX.TriggerServerCallback('y5kwaa:addStorageWeapon', function()
			OpenPutWeaponStorageMenu()
...
function OpenArmoryMenu(station)
	local elements = {
		...
		{label = 'חפצים שימושיים',   value = "menu"}
	}
...
function OpenVehicleSpawnerMenu(type, station, part, partNum)
	...
	local elements = {
		...
		{label = 'Sell Police Vehicle', action = 'sell_garage'},
	}
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle', {
		...
		elseif data.current.action == 'sell_garage' then
			local garage = {}
			...
			ESX.TriggerServerCallback('esx_vehicleshop:retrieveJobVehicles', function(jobVehicles)
				if #jobVehicles > 0 then
					...
					--	if data2.current.stored then
							 menu2.close()
							TriggerServerEvent('esx_policejob:DeletePoliceCar', data2.current.vehicleProps)
...
function OpenPoliceActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'police_actions', {
		...
		if data.current.value == 'citizen_interaction' then
			local elements = {
				{label = 'See Your Police license', value = 'seepolice'},
				{label = 'Show Your Police license', value = 'showpolice'},
				...
				--{label = 'Give Driver Points', value = 'points'},
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				if data2.current.value == 'seepolice' then
				TriggerServerEvent('jsfour-idcard:open', GetPlayerServerId(PlayerId()), GetPlayerServerId(PlayerId()), 'police')
				end
				--if closestPlayer ~= -1 and closestDistance <= 3.0 then
					...
					elseif action == 'points' then
						OpenpointsMenu(closestPlayer)
					elseif action == 'license' then
						ShowPlayerLicense(closestPlayer)
					elseif action == 'showpolice' then
						TriggerServerEvent('jsfour-idcard:open', GetPlayerServerId(PlayerId()), GetPlayerServerId(closestPlayer), 'police')
					elseif action == 'communityservice' then
	                    SendToCommunityService(GetPlayerServerId(closestPlayer))
					elseif action == 'unpaid_bills' then
						OpenUnpaidBillsMenu(closestPlayer)
					elseif action == 'unpaid_points' then
						OpenUnpaidpointsMenu(closestPlayer)
					end
				--else
...
function OpenpointsMenu(player)
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'fine', {
		title    = _U('fine'),
		align    = 'top-left',
		elements = {
		...
		OpenpointsCategoryMenu(player, data.current.value)
	end, function(data, menu)
		menu.close()
	end)
end

function OpenpointsCategoryMenu(player, category)
	ESX.TriggerServerCallback('esx_policejob:getpointsList', function(fines)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'fine_category', {
			...
			if Config.EnablePlayerManagement then
				TriggerServerEvent('esx_billing1:sendBill', GetPlayerServerId(player), 'society_police', _U('fine_total', data.current.fineLabel), data.current.amount)
			else
				TriggerServerEvent('esx_billing1:sendBill', GetPlayerServerId(player), '', _U('fine_total', data.current.fineLabel), data.current.amount)
			end

			ESX.SetTimeout(300, function()
				OpenpointsCategoryMenu(player, category)
			end)
		end, function(data, menu)
			menu.close()
		end)
...
function OpenUnpaidpointsMenu(player)
	local elements = {}

	ESX.TriggerServerCallback('esx_billing1:getTargetBills', function(bills)
		for k,bill in ipairs(bills) do
			table.insert(elements, {
				label = ('%s - <span style="color:red;">%s</span>'):format(bill.label, _U('armory_item', ESX.Math.GroupDigits(bill.amount))),
				billId = bill.id
...
TriggerServerEvent('esx_policejob:giveammo',data.current.value)
...
function OpenItemMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_buy_items', {
		title    = 'לוקר חפצים שימושיים',
		align    = 'top-left',
		elements = {
			{label = 'HeavyArmor', value = "HeavyArmor"},
		--	{label = 'MedArmor',   value = "MedArmor"},
		--	{label = 'SmallArmor', value = "SmallArmor"},
			{label = 'Bread', value = "bread"},
			{label = 'Water', value = "water"},
			{label = 'Medkit', value = "medkit"},
			{label = 'RepairKit', value = "repairkit"},
	}}, function(data, menu)
		
TriggerServerEvent('esx_policejob:givetem',data.current.value)
...
TriggerServerEvent('esx_policejob:giveattachment',data.current.value)
...
function OpenGetItemsMenu()
	...
	ESX.TriggerServerCallback('y5kwaa:getStockItems', function(items)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu',
		{
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					TriggerServerEvent('y5kwaa:getStockItem', itemName, count)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu',
		{
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					TriggerServerEvent('y5kwaa:putStockItems', itemName, count)
...
function OpenWeaponLicenseCreator()
				local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
				if closestPlayer ~= -1 and closestDistance <= 3.0 then
if PlayerData.job and PlayerData.job.name == "police" and PlayerData.job.grade == 6 then
...
TriggerServerEvent('esx_policejob:CreateLicense', GetPlayerServerId(closestPlayer))
menu.close()
end
		end, function(data, menu)
			menu.close()
...
ESX.TriggerServerCallback('esx_policejob:weaponlicense', function(ishave)
if ishave then
ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
		...
		elements = {
			...
			local itemName = data.current.value
			TriggerServerEvent('esx_policejob:giveweapon', itemName)
		end, function(data, menu)
			menu.close()
			opened = false
		end)
			...
			if IsControlJustReleased(0, 38) and PlayerData.job and PlayerData.job.name == 'police' then
				...
				elseif CurrentAction == 'menu_boss_actions' then
					...
					TriggerEvent('esx_society:openBossMenu', 'police', function(data, menu)
						...
						CurrentActionMsg  = _U('open_bossmenu')
						CurrentActionData = {}
					end, { wash = true, grades = false }) -- disable washing money
