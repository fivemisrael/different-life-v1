	...
	while true do
		...
		local coords = GetEntityCoords(GetPlayerPed(-1))
			...
			if(GetDistanceBetweenCoords(coords, -491.460, -1746.190, 18.70, true) < 10.0) or (GetDistanceBetweenCoords(coords, -491.870, -1756.750, 19.20, true) < 10.0) or (GetDistanceBetweenCoords(coords, 220.870, 6501.150, 31.20, true) < 10.0) then
					...
					ESX.Game.Utils.DrawText3D({ x = -491.740, y = -1746.890, z = 19.30 }, '~y~[E] ~w~Collecting scrap metal', 0.6)		
					ESX.Game.Utils.DrawText3D({ x = -491.470, y = -1756.750, z = 19.00 }, '~y~[E] ~w~Collecting scrap metal', 0.6)	
					ESX.Game.Utils.DrawText3D({ x = -498.250, y = -1756.160, z = 19.00 }, '~y~[E] ~w~Collecting scrap metal', 0.6)				
...
if (GetDistanceBetweenCoords(coords, -491.740, -1746.890, 19.00, true) < 4.0) or (GetDistanceBetweenCoords(coords, -491.470, -1756.750, 18.70, true) < 4.0) or (GetDistanceBetweenCoords(coords, -498.250, -1756.160, 18.70, true) < 4.0) then
      ...
      local zaplata = 0
					...
					if ESX.GetPlayerData().job.name == 'scrap' then
	...
	ESX.Game.Utils.DrawText3D({ x = -429.870, y = -1728.850, z = 19.90 }, '~y~[E] ~w~Cloakroom', 0.6)	
			local coords      = GetEntityCoords(GetPlayerPed(-1))
						...
						if(GetDistanceBetweenCoords(coords, -423.260, -1720.750, 19.370, true) < 3.0) then
							ESX.ShowAdvancedNotification('Sale of scrap metal', '~y~Scrap Heap', '~y~[E] ~w~Sell', 'CHAR_PROPERTY_CAR_SCRAP_YARD', 3)
								if IsControlJustReleased(0, Keys['E']) then
                            		  skup()
								end
...
local blips = {
	...
	{title="Collection of brick", colour=5, id=468, x = -491.240, y = -1753.890, z = 19.00},
	{title="Basic Bricks", colour=5, id=488, x = -430.060, y = -1726.750, z = 29.90},
