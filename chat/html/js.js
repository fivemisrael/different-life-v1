var settings = {messages: 0, timer: null, inchat: false, scrolldown: true, render: false, hide: false, hidechat: 0, start: 0, suggests: [], mymessages: [], onmessage: -1, commands: []}
addEventListener("message", function(e)
{
	...
	if(item.meta)
	{
		...
		{
			for(var i = 0; i < item.suggests.length; i++)
			{
				var found = false;
				var j = 0;
		...
		{
			for(var i = 0; i < settings.suggests.length; i++)
			{
				if(settings.suggests[i].name == item.name)
				{
					delete settings.suggests[i];
					for(var j = i; j < settings.suggests.length-1; j++)
		...
		{
			var dspan = "<span style=\"color: "+item.color+";"+(item.style ? (item.style):(""))+"\">";
			...
			var maxscroll = elem.scrollHeight-Math.round(0.18*$(window).height());
			var nowtop = elem.scrollTop;
...
addEventListener("keydown", function(e)
{
	if(settings.inchat)
	{
		if(e.which == 13)
		{
			...
			{
				var found = false;
				for(var i = 0; i < settings.lastmessages.length && !found; i++)
					if(settings.lastmessages == input.value)
					{
						if(found)
						{
							for(var j = i; j < setting.lastmessages.length-1; j++)
				...
				if(!found)
				{
					for(var i = 0; i < settings.lastmessages.length; i++)
...
function OnFrame(gtime)
{
	if(settings.render && settings.hidechat != -1)
	{
		...
		{
			var htime = (settings.hidechat-gtime)/2000;
			if(htime < 0)
				htime = 0;
...
function ClearUp(chat, num1, num2)
{
	...
	{
		var find = chat.includes("<br>");
		if(find)
		{
			var g = chat.indexOf("<br>");
	...
	document.getElementById("input").oninput = function(event)
	{
		...
		if(value.startsWith("/"))
		{
			...
			while(i < settings.suggests.length && found < maxh)
			{
				...
				{
					var help = settings.suggests[i].help || "";
					if(spl.length <= 1)
